...
-- TODO: Remove, SEE: https://github.com/neovim/neovim/pull/32346
set.diffopt:append({
...
set.fillchars = {
  horiz = "━", -- digraph `HH`
  horizup = "┻", -- digraph `HU`
  horizdown = "┳", -- digraph `HD`
  vert = "┃", -- digraph `VV`
  vertleft = "┫", -- digraph `VL`
  vertright = "┣", -- digraph `VR`
  verthoriz = "╋", -- digraph `VH`
  -- fold = "∧", -- ∧∧∧∧∧∧∧∧∧∧ digraph `AN`
  -- fold = "♯", -- ♯♯♯♯♯♯♯♯♯♯ digraph `MX`
  -- fold = "※", -- ※※※※※※※※※※ digraph `:X`
  -- fold = "󰘴", -- 󰘴󰘴󰘴󰘴󰘴󰘴󰘴󰘴󰘴󰘴 nerdfont nf-md-apple_keyboard_control
  -- fold = "", --  nf-cod-chevron_up
  fold = "", --  nerdfont nf-oct-chevron_up
  foldopen = "▼", -- digraph `Dt`
  foldclose = "▶", -- digraph `PR`
  diff = "╱", -- digraph `FD`
...
set.guicursor:append("c:hor20") -- horizontal in normal and command mode, block in operator-pending mode
set.ignorecase = true -- use case-insensitive in search
-- set.iskeyword:remove({ "_" })
-- set.laststatus = 3 -- use global status line
-- TODO: Configure 'list' and 'listchars'
-- set.list = true
-- set.listchars:append({
--   tab = "┃ ",
-- })
-- TODO: https://github.com/neovim/neovim/pull/31492
...
set.showcmd = true -- hide (partial) command in the last line of the screen
...
-- set.statuscolumn = "%C%=%( %{v:virtnum != 0 ? '' : v:relnum ? v:relnum : &number ? v:lnum : ''} %)%s"
...
set.title = true
-- TODO: set 'titlestring' to show terminal name
set.undofile = true -- save undo history to an undo file on BufWrite
set.updatetime = 2000 -- if this many ms nothing is typed, write the swap file
set.visualbell = true
...
set.wildmode = { "longest", "full" } -- TODO: Add explanation
set.wildoptions:remove({ "tagfile" }) -- TODO: Add explanation or remove
-- set.winborder = "soid"
